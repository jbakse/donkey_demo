class Rectangle {
  ...
  sliceDown(sliceCount) {
    ...
    for (let i = 0; i < sliceCount; i++) {
      slices.push(new Rectangle(this.x, this.y + i * sliceHeight, this.w, sliceHeight));
    }
    return slices;
  }
  ...
  sliceRight(sliceCount) {
    ...
    for (let i = 0; i < sliceCount; i++) {
      slices.push(new Rectangle(this.x + i * sliceWidth, this.y, sliceWidth, this.h));
    }
    return slices;
  }
...
function fillSpaceBooks(space) {
  ...
  let spaceHeight = random(.25);
  bookRects.forEach((bookRect) => {
    if (random() < .5) {
      spaceHeight = random(.25);
    }
...
function drawBook(r) {
  ...
  // text
  const textTop = random(.2, .4) * r.h;
  const textBottom = random(.5, .8) * r.h;
  noFill();
  if (random() < .8) {
    fill(1, 0, 1);
  }
  ...
  for (let y = textTop; y < textBottom; y += 4) {
    let width = random() < .25 ? .5 : .25;
    rect(r.x + r.w * .75, r.y + y, r.w * -width, 2);
  }

  // bottom flair
  if (random() < .1) {
    fill(noise(0, r.y * colorFreq, .5) * 2, 1, 1);
